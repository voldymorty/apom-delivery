...
class _DeliveryDetailsScreenState extends ConsumerState<DeliveryDetailsScreen>
  ...
  //   final locationEnabled = await _checkLocationService();
  //
  ...
  Future<bool> _checkAppLocationPermission() async {
    ...
    if (permission == LocationPermission.denied) {
      CustomSnackBar.error(context, 'Location permission is required.');
      return false;
    }
  ...
  Future<void> _startCapture() async {
    try {
      ...
      debugPrint("LOCATION PERMISSION STATUS BEFORE: $permission");

      if (permission == LocationPermission.denied) {
        permission = await Geolocator.requestPermission();
      }

      debugPrint("LOCATION PERMISSION STATUS AFTER: $permission");

      if (permission == LocationPermission.denied) {
        CustomSnackBar.error(context, "Location permission denied");
        return;
      }
      ...
      // Permission allowed, now capture photo
      final XFile? photo = await _picker.pickImage(
        source: ImageSource.camera,
        imageQuality: 70,
      );
      ...
      if (mounted) {
        CustomSnackBar.error(context, "Camera error: $e");
      }
    }
  }
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text(
            "Please turn on your device location to capture the proof photo.",
          ),
          actions: [
            TextButton(
              onPressed: () {
            ...
            ElevatedButton(
              ...
              child: const Text("Enable Location",style: TextStyle(color:AppColors.deliveryColor),),
            ),
          ],
        );
      },
  ...
  Future<bool> _checkLocationService() async {
    ...
    if (!serviceEnabled) {
      final allow = await _showLocationDialog();

      if (!allow) return false;

      await Geolocator.openLocationSettings();

      // Wait until user enables GPS
